...
| [`claude-on-lcrc/`](claude-on-lcrc/) | Guide for running Claude Code (CLI + VSCode plugin) on Bebop via `argo-shim`, including the compute-node tunnel workflow. See [claude-on-lcrc.md](claude-on-lcrc/claude-on-lcrc.md). Includes [`start-argo-shim.sh`](claude-on-lcrc/start-argo-shim.sh) to launch `argo-shim` (or `--tunnel`) in a tmux session in one step, or `--all` for both at once. |
